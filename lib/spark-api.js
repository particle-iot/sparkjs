 ...
 * @param {object} params - Params to be sent when resolved
 * @param {function} callback - Function to execute on complete
 * @returns: null
 */
SparkApi.prototype.resolveDefer = function(defer, err, body, params, callback) {
  if (err) {
    params = null;
    defer.reject(err);
...
SparkApi.prototype.login = function (username, password, callback) {
  ...
  var handler = function(err, responses, body) {
    var params = body.access_token;
    var result = this.resolveDefer(defer, err, body, params, function() {
      this.accessToken = body.access_token;
    }.bind(this));

    this.emitAndCallback('login', result.err, result.params, callback);
  }.bind(this);
...
SparkApi.prototype.defaultHandler = function(eventName, defer, callback) {
  var handler = function(err, response, body) {
    this.resolveDefer(defer, err, body, body);
...
SparkApi.prototype.listDevices = function (callback) {
  ...
  var handler = function (err, response, body) {
    var devices = Array.isArray(body) ? body : [];
    var result = this.resolveDefer(defer, err, body, devices, function() {
      this.devices = devices;
    }.bind(this));

    this.emitAndCallback('listDevices', result.err, result.params, callback);
  }.bind(this);
...
SparkApi.prototype.createUser = function (username, password, callback) {
  ...
  if (!username || (username === '') || (!emailRegex.test(username))) {
    ...
    var handler = function (err, response, body) {
      var result = this.resolveDefer(defer, err, body, body, function() {
        this.username = username;
        this.password = password;
      }.bind(this));

      this.emitAndCallback('createUser', result.err, result.params, callback);
    }.bind(this);
