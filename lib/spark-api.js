 ...
 * @returns: null
 */
SparkApi.prototype.resolveDefer = function(defer, err, data, callback) {
  if (defer) {
    if (err) {
      defer.reject(err);
    } else{
 ...
 * @returns: null
 */
SparkApi.prototype.createDefer = function(eventName, callback) {
  var defer = null;
 ...
 */
SparkApi.prototype.normalizeErr = function(err, data, callback) {
  if (!err && data.error) {
 ...
 */
SparkApi.prototype.defaultHandler = function(eventName, defer, userCb, sparkCb) {
 ...
 * @returns null
 */
SparkApi.prototype.emitAndCallback = function(eventName, err, data, callback) {
  if (!!err) { data = null; }
...
SparkApi.prototype.listDevices = function (callback) {
  ...
  var handler = function (err, response, data) {
    err = this.resolveDefer(defer, err, data, function(devices) {
      this.devices = Array.isArray(devices) ? devices : [];
    }.bind(this));

    this.emitAndCallback('listDevices', err, data, callback);
  }.bind(this);
...
SparkApi.prototype.createUser = function (username, password, callback) {
  ...
  if (!username || (username === '') || (!emailRegex.test(username))) {
    ...
    var handler = function (err, response, data) {
      err = this.resolveDefer(defer, err, data, function() {
        this.username = username;
        this.password = password;
      }.bind(this));

      this.emitAndCallback('createUser', err, data, callback);
    }.bind(this);
...
SparkApi.prototype.downloadBinary = function (url, filename, callback) {
 ...
 console.log("grabbing binary from: " + this.baseUrl + url + '?access_token=' + this.accessToken);
