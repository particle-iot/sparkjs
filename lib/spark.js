...
/**
 * List access tokens from the Spark cloud for the specified user.
 *
 * @this {Spark}
 * @param {string} username - Email for the user to be registered
 * @param {string} password
 ...
 */
Spark.prototype.listAccessTokens = function (username, password, callback) {
  var defer = this.createDefer('listAccessTokens', callback),
      handler = this.defaultHandler('listAccessTokens', defer, callback).bind(this);

  this.api.listAccessTokens(username, password, handler);
...
Spark.prototype._createStreamProcessor = function() {
  ...
  Processor.prototype = {
    chunks: [],
    linePart: '',
    appendToQueue: function(data) {
      if (this.linePart) {
        data = this.linePart + data;
        this.linePart = '';
      }
      var lastNlIndex = data.lastIndexOf('\n');
      var arr = data.split('\n');
      if (lastNlIndex !== data.length - 1) {
        this.linePart = arr.pop();
      }
